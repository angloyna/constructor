...
def _find_out_of_date_precs(precs, channel_urls, platform):
    ...
    for prec in precs:
        ...
        if prec.name == 'openssl':
            print(all_packages)
        most_recent = max(all_packages, key=lambda package_version: (parse_version(package_version.version), package_version.build_number, package_version.timestamp) if not isinstance(parse_version(package_version.version), LegacyVersion) else (parse_version(package_version.version).base_version, package_version.build_number, package_version.timestamp))
